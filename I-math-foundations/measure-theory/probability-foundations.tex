...
\begin{proof}
\textbf{(1)} Define $B_1 = A_1$ and $B_n = A_n \setminus A_{n-1}$ for $n \geq 2$. The $B_n$ are disjoint, $A_n = \bigcup_{k=1}^n B_k$, and $\bigcup_{n=1}^\infty A_n = \bigcup_{n=1}^\infty B_n$. By countable additivity:
\begin{equation}
    \Prob\!\left(\bigcup_{n=1}^\infty A_n\right) = \sum_{n=1}^\infty \Prob(B_n) = \lim_{N \to \infty} \sum_{n=1}^N \Prob(B_n) = \lim_{N \to \infty} \Prob(A_N).
\end{equation}

\textbf{(2)} Apply (1) to $A_n^c$: these increase, so $\Prob(\bigcup A_n^c) = \lim \Prob(A_n^c)$. By De Morgan, $\bigcup A_n^c = (\bigcap A_n)^c$, giving $1 - \Prob(\bigcap A_n) = \lim(1 - \Prob(A_n))$.
\end{proof}
