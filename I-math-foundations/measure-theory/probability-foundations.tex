...
Let $i < j$. We must show $B_i \cap B_j = \emptyset$.
Since $A_1 \subseteq A_2 \subseteq \cdots$, we have $A_i \subseteq A_{j-1}$. By definition $B_i \subseteq A_i$ (either $B_1 = A_1$ or $B_i = A_i \setminus A_{i-1} \subseteq A_i$), so $B_i \subseteq A_{j-1}$.
But $B_j = A_j \setminus A_{j-1}$, meaning $B_j \cap A_{j-1} = \emptyset$. Since $B_i \subseteq A_{j-1}$, we conclude $B_i \cap B_j = \emptyset$.
